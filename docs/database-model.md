...
# Дополнение: локальный кэш Naumen/NCC

Для read-only NCC интеграции добавлены/используются поля и таблицы:

- `naumen_projects.naumen_customer_uuid` — UUID партнёра NCC для фильтрации по `partneruuid`;
- `naumen_projects.naumen_project_uuid` — optional дочерний проект;
- `naumen_projects.manual_stats_enabled` — ручной режим статистики, если UUID не указан;
- `queues.queue_uuid`, `queues.source_system`;
- `workload_intervals.source_system`, `workload_intervals.import_run_id`;
- `naumen_operators.metrics_data`;
- `ncc_operator_interval_stats` — операторская нагрузка по часу, очереди и login;
- `ncc_forecast_profiles` — профиль нагрузки по дню недели, часу и очереди;
- `naumen_sync_runs.partner_uuid`, `period_begin`, `period_end`, `rows_by_type`.

ИНН активного неархивного сотрудника защищён partial unique index `ux_active_employees_inn`.
